...
if ( isset($_GET['Unapprove_ID'])) {
	if ( !empty($_GET['Unapprove_ID'])) {
		$sql = "UPDATE comment SET status ='disapproved'  WHERE id = '$_GET[Unapprove_ID]'";
		$exec = Query($sql);
		if ( $exec) {
			...
			<div class="col-xs-10">
				<div>
					...
					<div class="table-responsive">
							<?php
							$sql = "SELECT * FROM comment WHERE status ='disapproved' ORDER BY date_time DESC LIMIT 25";
							$exec = Query($sql);
							$postNo = 1;
							if(mysqli_num_rows($exec) < 1	) {
								?>
									<p class="lead">No Disapproved Comments</p>
								<?php
							}else{ ?>
							<span class="lead">Disapproved Comments</span>
							...
							<?php
								while ($post = mysqli_fetch_assoc($exec)) {
									...
									<td><?php echo formatDate($comment_dateTime); ?></td>
					...
					<div class="table-responsive">
							<?php
							$sql = "SELECT * FROM comment WHERE status ='approved' ORDER BY date_time DESC LIMIT 20";
							$exec = Query($sql);
							$postNo = 1;
							if(mysqli_num_rows($exec) < 1	) {
								?>
							...
							<?php
								while ($post = mysqli_fetch_assoc($exec)) {
									...
									<td><a href="Comments.php?Unapprove_ID=<?php echo $comment_id; ?>"><button class="btn btn-danger">Disapprove</button></a></td>
									<?php
									$postNo++;
								}
							}
